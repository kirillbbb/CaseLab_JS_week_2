...
describe('API', () => {
  ...
  describe('Equipment', () => {
    ...
    it('returns 422 for invalid sort order', async () => {
      const response = await request(app).get(
        '/api/equipment?sortOrder=random',
      );
    ...
    it('updates equipment', async () => {
      ...
      const createdEquipment = createResponse.body.data;

      const response = await request(app)
        .patch(`/api/equipment/${createdEquipment.id}`)
        .send({
          name: 'New Name',
        });
